...
measure every `SENSOR_POLL_INTERVAL` and publishes it through `OUT`, as two runs of input registers
under one lock so every value describes the same moment. It shares the Modbus mutex with
`fan_control_routine` and yields to it: a failed poll is logged and dropped rather than retried,
because the next poll carries fresher values than a retry would and a silent fan would otherwise
hold the mutex through a run of timeouts. The fan's maximum speed (`D119`, a holding register) is
read once and cached, because every speed the fan reports is a fraction of it; until it is known
the reading reports the speed as `null` rather than withholding the values that do not depend on
it. Besides the fan's own speed, temperatures and power, the second run carries what the air is
doing: the temperature and humidity of the sensor wired to the fan (`D02E`/`D02F`) and the volume
flow through it (`D033`). Those two sensor values are tenths rather than whole units, and are
carried as tenths in `fan_sensor::Tenths` rather than as floats — the RP2040 has no floating point
unit. The documented energy counter (`D029`/`D02A`) is *not* read: both fans answer `0xFFFF` for
it, so the sensor was removed rather than announced as a value that is never real.
